...
class ImageLabel(QtGui.QLabel) :
    ...
    def setSpritePatch(self, spritePatch) : 
        self.spritePatch = spritePatch.copy()
        self.update()
        
    def setLines(self, lines, selectedPoint=None) :
        self.lines = lines
...
#                                [ 1269.7,   595. ,  1069.7,   128. ]], dtype=float)
        ...
        self.trajectory = None
        self.trajectoryInTopDown = None
        self.orientations = None #np.zeros(len(self.trajectoryInTopDown))
        ...
        self.bgImage = np.ascontiguousarray(Image.open(dataPath+dataSet+"median.png"))
...
#         print trajSize, trajSize+self.trajectorySizeDelta
        ...
        self.trajectory = smoothedPath
        ## and get it's top down projection
        self.trajectoryInTopDown = np.dot(np.linalg.inv(self.homography), np.array([self.trajectory[:, 0], self.trajectory[:, 1], np.ones(len(self.trajectory))]))
        self.trajectoryInTopDown /= self.trajectoryInTopDown[-1, :]
        self.trajectoryInTopDown = self.trajectoryInTopDown[0:2, :].T
    ...
    def updateOrientations(self) :
        if self.trajectoryInTopDown != None :
            self.orientations = np.zeros(len(self.trajectoryInTopDown))
    ...
    def updateOriginalView(self) :
        ...
        ## set sprite trajectory
        if self.trajectory != None :
            self.originalImageLabel.setTrajectory(self.trajectory)
        ...
        projectedPoints = np.dot(self.homography, gridPoints)
        projectedPoints /= projectedPoints[-1, :]
        self.originalImageLabel.setPlaneGrid(projectedPoints[0:2, :].T)
        
    def updateTopDownView(self) :
        
        ## get morphed background using the inverse of the homography and show it in the top down label
        morphedBackground = np.ascontiguousarray(cv2.resize(cv2.warpPerspective(bgImage, np.linalg.inv(self.homography), (5000, 3000)), 
        ...
        if self.trajectory != None :
            self.topDownImageLabel.setTrajectory(np.dot(self.trajectoryInTopDown, self.topDownScaling))
    ...
    def changeFrame(self, idx) :
        ...
        if self.orientations != None and self.trajectoryInTopDown != None :
            bboxCenter = self.trajectoryInTopDown[idx, :]
    ...
    def changeDistortionParameter(self) :
        ...
        self.undistortedBgImage = np.ascontiguousarray(cv2.undistort(self.bgImage, self.cameraMatrix, self.undistortParameters))
        ## update undistorted trajectory
        
        self.updateViewsBasedOnData()
        
    def updateViewsBasedOnData(self) :
        ...
        if self.doSetGroundPlaneCheckBox.isChecked() :
            ...
            self.originalImageLabel.setTrajectory(cv2.undistortPoints(self.trajectory.reshape((1, len(self.trajectory), 2)),
    ...
    def createGUI(self) :
        ...
        self.doSetGroundPlaneCheckBox.stateChanged.connect(self.updateViewsBasedOnData)
...
print np.dot(np.linalg.inv(window.homography), np.array([window.rectangleCorners[:, 0], window.rectangleCorners[:, 1], np.ones(len(window.rectangleCorners))]))
